...
ReceivablePacket::ReceivablePacket(uint8_t* bytes)
{
	readBuffer = bytes;
...
FString ReceivablePacket::ReadString()
{
...
uint8_t ReceivablePacket::ReadByte()
{
	return readBuffer[readBufferPosition++];
}

uint16_t ReceivablePacket::ReadShort()
{
	uint8_t* byteArray = new uint8_t[2];
	byteArray[0] = ReadByte();
	byteArray[1] = ReadByte();
...
uint32_t ReceivablePacket::ReadInt()
{
...
uint64_t ReceivablePacket::ReadLong()
{
